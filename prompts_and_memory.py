...
class PromptsAndMemoryMixin:
    def _ensure_prompt_files(self):
        templates = {
            ...
            "PCF.md": "You are the NoClaw Symbolic Fixer taking over Phase 3 Cascaded Edits.\n{memory_section}We just modified `{trigger_file}`, and it broke a dependency downstream: `{rel_broken_path}`.\n\n### Linter Errors for `{rel_broken_path}`:\n{mypy_errors}\n\n### Source Code of `{rel_broken_path}`:\n```python\n{broken_code}\n```\n\n### Instructions:\n1. Respond EXCLUSIVELY with a <THOUGHT> block followed by ONE <EDIT> block to fix the broken references.\n2. **MYPY UNION/NONE ERRORS (CRITICAL)**: If the error contains `[union-attr]` or states `Item \"None\" of ... has no attribute`, it means a variable might be `None`. You MUST fix this by wrapping the operation in an `if variable is not None:` check. Do not just remove the type hint.\n\n### REQUIRED XML FORMAT:\n<THOUGHT>\nExplanation of cascade fix...\n</THOUGHT>\n<EDIT>\n<SEARCH>\nExact lines to replace\n</SEARCH>\n<REPLACE>\nNew replacement lines\n</REPLACE>\n</EDIT>",
            
            "PIR.md": "You are an elite NoClaw developer performing a post-implementation repair.\nAn automated attempt to implement a feature or bugfix has failed, resulting in the following errors.\n\n### Original Goal / Change Request:\n{context_of_change}\n\n### The Resulting Errors (Linter/Runtime):\n{err_text}\n\n### The Broken Code in `{rel_path}`:\n```\n{code}\n```\n\n### Instructions:\n1. Analyze the 'Original Goal' and the 'Resulting Errors' together.\n2. CRITICAL INDENTATION CHECK: If the error says `Unexpected indentation` or `Expected an indented block`, the previous AI misaligned the Python spaces. Look at the surrounding methods, count the spaces, and rewrite the <EDIT> block with PERFECT absolute indentation.\n3. CRITICAL IMPORT CHECK: If the error is `F821 Undefined name` or `ModuleNotFoundError`, you MUST create an <EDIT> block at the top of the file to add the missing `import` statement.\n4. NONE-TYPE ERRORS: If the error complains about an object being `None` or having no attribute (e.g., `[union-attr]`), ensure you add an `if object is not None:` safety check.\n5. Create a surgical XML edit to fix the code.\n\n### REQUIRED XML FORMAT:\n<THOUGHT>\nRoot Cause: ...\nImports Needed: ...\nIndentation Fix Needed: [Yes/No]\nAction: ...\n</THOUGHT>\n<EDIT>\n<SEARCH>\nExact lines to replace\n</SEARCH>\n<REPLACE>\nNew replacement lines\n</REPLACE>\n</EDIT>",
        }
